// ─── marketing-storyboard / ErrorBoundary.tsx ────────────────────────────────
//
// React error boundary for the Konva-backed marketing canvas.
//
// Konva mount failures (WebGL unavailable, OOM on large campaigns,
// react-konva version skew) currently leave the page blank — Astro hydrates
// the `client:only="react"` island and on a thrown render error the island
// silently unmounts. The user just sees navy.
//
// The campaign handoff page is SSG'd HTML — no React/Konva needed — so the
// graceful fallback is: "canvas broke, read the launch brief instead."
// ─────────────────────────────────────────────────────────────────────────────
...
interface Props {
  ...
  /** Link to the SSG'd handoff brief for the current campaign. */
  handoffHref?: string;
...
export default class ErrorBoundary extends React.Component<Props, State> {
  ...
  componentDidCatch(err: unknown): void {
    if (typeof console !== 'undefined') {
      console.error('[MarketingStoryboardCanvas] render failed:', err);
    }
  }
      ...
      <div role="alert" style={fallbackStyles.root}>
        <div style={fallbackStyles.card}>
          ...
          <p style={fallbackStyles.body}>
            The interactive campaign board could not be rendered in this
            browser. Your campaign data is unchanged — only the canvas view
            failed.
          </p>
          {href && (
            <a href={href} style={fallbackStyles.cta}>
              View the launch brief →
            </a>
          )}
          {this.state.message && (
