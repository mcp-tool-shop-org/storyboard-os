...
<p align="center"><strong>Storyboard OS用の、ドメイン設定可能なKonvaキャンバスレンダラー。</strong></p>

---

# @storyboard-os/canvas

インタラクティブなストーリーボード作成のための、Konvaを利用したキャンバスレンダラー。フレーム、接続、選択、ドラッグ、タイプバッジ、および接続ラベルの描画を行います。**RPG、脚本、その他のドメイン固有の用語に関する知識は一切持っていません**。すべての視覚的な設定は、利用するアプリケーションによって注入されます。

別の用途（脚本、テーブルトークゲーム、ゲームマップなど）でも、独自の構成を渡すことで、このパッケージを変更せずに完全に機能するキャンバスを利用できます。
...
`StoryboardCanvasConfig`は、キャンバスがあなたのドメインについて知っておく必要のある唯一の情報です。
...
ドメインは、キャンバスがその意味を知らなくても、フレームカードにバッジチップを付加できます。
...
`rpg-storyboard`では、`@storyboard-os/rpg-domain`の`getFrameBadges(frame, connections)`がこれらのバッジを生成します。キャンバスは、それらが「STATE」または「SPEC」を意味することを理解せずに、それらをレンダリングします。

---

## ビューポートハンドル

`StoryboardCanvas`は`forwardRef`コンポーネントです。`ref`を渡すことで、`ViewportHandle`を取得できます。
...
|---|---|
| ドラッグ時の背景 | パン |
| Ctrl/Cmd + スクロールホイール | カーソルの位置でのズーム |
| 通常のスクロール | パン（自然な2本指トラックパッド操作） |
| フレームのドラッグ | フレームを再配置します。`onFramePositionChange`をリリース時にトリガーします。 |
| フレームのクリック | フレームを選択します。`onSelectFrame`をトリガーします。 |
| 接続のクリック | 接続を選択します。`onSelectConnection`をトリガーします。 |
| 背景のクリック | 選択を解除します。`onSelectFrame(null)`をトリガーします。 |

背景ドラッグガード（`e.target !== stage`）は、フレームカードがドラッグされているときにパンがトリガーされるのを防ぎます。

---

## コンテナのサイズ調整

`StoryboardCanvas`は`ResizeObserver`を使用して、コンテナのサイズを測定し、完全に埋めます。明示的な`width`または`height`プロパティは渡さないでください。コンテナにサイズを設定するだけです。
...
## ビューポートの計算 — スタンドアロンのユーティリティ

ビューポートの計算関数は、純粋であり、ReactまたはKonvaへの依存はありません。これらの関数は、キャンバスコンポーネントの外部でレイアウトまたは配置を計算する必要があるアプリケーションのためにエクスポートされています。
...
`viewport.test.ts`にあるすべての27個のビューポート計算テストは、DOMまたはKonvaなしで実行されるため、高速で信頼性が高く、CI環境で利用できます。
...
`@storyboard-os/canvas`は、`@storyboard-os/core`、`@storyboard-os/rpg-domain`、または他のアプリケーションからインポートしません。ドメイン設定はプロパティを通じて渡され、キャンバスはドメインレイヤーに直接アクセスしません。

検証：このパッケージのソースコード内で`rpg-domain`、`quest`、`npc_beat`、または`stateChange`を検索しても、何も返ってこないことが確認されています。

---

## 信頼モデル

`@storyboard-os/canvas`はReactコンポーネントライブラリです。ネットワークアクセス、localStorageへの読み書き、サーバーサイドの効果、およびテレメトリーはありません。すべての永続化は、`onFramePositionChange`を介して、利用するアプリケーションの責任です。
